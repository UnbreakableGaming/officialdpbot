    ...
    message.delete()
        ...
        function embed (msg) {
            let emb = new Discord.RichEmbed()
                ...
                .setAuthor(`${message.guild.name} - Help`, client.user.avatarURL)
                .setDescription(`${msg}
                                \n Hello This Bot Is Still Beta If theres any bug please join support server and dm Unbreakable!`)
                .setThumbnail(client.user.avatarURL)
                .setFooter(`${client.config.footer}\n${message.guild.name} Utilities ➤ Command ran by ${message.author.username}`)
            return emb
        }
        ...
        m.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] })
            .then(collected => {
                ...
                if (reaction.emoji.name === '🔒') {
                  o += `**${message.guild.name} Moderation Commands**\n\n`
                  ...
                  o += `**${message.guild.name} Fun & Games Commands**\n\n`
                  o += `\`${p}8ball <future>\`\n`
                  o += `\`${p}flip\`\n`
                  o += `\`${p}hug <user>\`\n`
                  o += `\`${p}joke\`\n`
                  ...
                  o += `**${message.guild.name} Support & Utilites Commands**\n\n`
